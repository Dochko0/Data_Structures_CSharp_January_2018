...
public class Judge : IJudge
{
    ...
    {
        return this.bySubmissionId.Values.Where(x => x.Points >= minPoints 
        && x.Points <= maxPoints && x.Type == submissionType);
    }

    public IEnumerable<int> ContestsByUserIdOrderedByPointsDescThenBySubmissionId(int userId)
    {
        return this.bySubmissionId.Values.Where(x => x.UserId == userId).OrderByDescending(x=>x.Points)
            .ThenBy(x=>x.Id)
    ...
    public IEnumerable<Submission> SubmissionsInContestIdByUserIdWithPoints(int points, int contestId, int userId)
    {
        ...
        x.UserId == userId && x.Points == points && x.ContestId == contestId);
    ...
    public IEnumerable<int> ContestsBySubmissionType(SubmissionType submissionType)
    {
        ...
        x.Type == submissionType).Select(x=>x.ContestId).Distinct();
    }
}
